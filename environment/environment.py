...
class CliffWalkEnvironment(BaseEnvironment):
    def env_init(self, env_info = {}):
        ...
        self.grid_w = env_info.get("grid_width", 7)

        self.start_loc = (self.grid_h - 1,0)
        self.goal_loc = (self.grid_h-1, self.grid_w-1)
        self.cliff = [(5,i) for i in range(1,self.grid_w-1)]+[(0,i) for i in range(1,self.grid_w-1)]
    ...
    def env_step(self, action):
        ...
        if self.agent_loc in [(1, i) for i in range(1, 6)] and action == 3:  # (1,1) to (1,5) and action == right
            if random.random() < 0.5:  # 50% probability
                action = 2  # Change action to 'down'
